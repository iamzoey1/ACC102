...
def load_data():
    ...
    # ---------- Data Cleaning ----------
    # 1. Missing values
    critical_cols = ["campaign_id", "estimated_reach", "engagements", "product_sales"]
    ...
    # 3. Logical bounds
    df = df[
        (df["estimated_reach"] > 0) &
        (df["engagements"] >= 0) &
        (df["product_sales"] >= 0)
    ]
    
    # 4. Outlier capping (1st / 99th percentile)
    for col in ["estimated_reach", "engagements", "product_sales"]:
...
try:
    df, raw_rows = load_data()
    final_rows = len(df)
    st.sidebar.success(f"✅ Data loaded: {final_rows} campaigns")
    st.sidebar.caption(f"Retention after cleaning: {final_rows/raw_rows:.1%}")
